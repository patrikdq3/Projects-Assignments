...
typedef enum {
oddsym = 1, identsym, numbersym, plussym, minussym, multsym, slashsym, eqsym, neqsym, lessym, leqsym, gtrsym, geqsym, lparentsym, rparentsym, commasym, semicolonsym, periodsym, becomessym, beginsym, endsym, ifsym, thensym, whilesym, dosym, callsym, constsym, varsym, writesym, readsym, fisym, procsym } token_type;
...
void errorMessage(FILE *inFile, FILE *outFile, int errorCode)
{
    ...
    switch(errorCode){
        ...
        case 404:
            ...
            break;
        case 505:
            message = "Error: no \"fi\" after \"then\"";
            break;
        case 1:
        ...
        case 21:
            ...
            break;
        case 22:
            printf("\nERROR 22: 'const', 'var', or 'procedure' must be followed by an identifier");
            fprintf(outputfile, "\nERROR 22: 'const', 'var', or 'procedure' must be followed by an identifier");
            sprintf(parseTemp, "\nERROR 22: 'const', 'var', or 'procedure' must be followed by an identifier");
            strcat(parseOut, parseTemp);
            break;
        case 23:
            printf("\nERROR 23: Incorrect symbol after procedure declaration");
            fprintf(outputfile, "\nERROR 23: Incorrect symbol after procedure declaration");
            sprintf(parseTemp, "\nERROR 23: Incorrect symbol after procedure declaration");
            strcat(parseOut, parseTemp);
            break;
        case 24:
            printf("\nERROR 24: Assignment to constant or procedure is not allowed");
            fprintf(outputfile, "\nERROR 24: Assignment to constant or procedure is not allowed");
            sprintf(parseTemp, "\nERROR 24: Assignment to constant or procedure is not allowed");
            strcat(parseOut, parseTemp);
            break;
        case 25:
            printf("\nERROR 25: 'call' must be followed by an identifier");
            fprintf(outputfile, "\nERROR 25: 'call' must be followed by an identifier");
            sprintf(parseTemp, "\nERROR 25: 'call' must be followed by an identifier");
            strcat(parseOut, parseTemp);
            break;
        case 26:
            printf("\nERROR 26: Call of a constant or variable is meaningless");
            fprintf(outputfile, "\nERROR 26: Call of a constant or variable is meaningless");
            sprintf(parseTemp, "\nERROR 26: Call of a constant or variable is meaningless");
            strcat(parseOut, parseTemp);
            break;
        case 27:
            printf("\nERROR 27: Expression must not contain a procedure identifier");
            fprintf(outputfile, "\nERROR 27: Expression must not contain a procedure identifier");
            sprintf(parseTemp, "\nERROR 27: Expression must not contain a procedure identifier");
            strcat(parseOut, parseTemp);
            break;
...
char *prog1()
{
    ...
    } //else { print in elf.txt FAIL COMPILATION}
    ...
    fclose(inputfile);
    fclose(outputfile);
    FILE* elfFile = fopen("elf.txt", "a"); // Open for appending to ensure it exists.
    if (elfFile) {
        fclose(elfFile);
    }
    return parseOut;
...
void printAssembly(const char* operation, int L, int M) {
    static FILE *elfFile = NULL;
    static int lineNum = 1; // Static variable to keep track of the instruction line numbers
    if (!elfFile){
    ...
    // Convert operation string to OP code.
    int opCode = 0;
    // Example conversion, implement conversions for all operations.
    if (strcmp(operation, "LIT") == 0) opCode = 1;
    else if (strcmp(operation, "OPR") == 0) opCode = 2;
    else if (strcmp(operation, "LOD") == 0) opCode = 3;
    else if (strcmp(operation, "STO") == 0) opCode = 4;
    // ... Add all other operations.

    fprintf(elfFile, "%d %d %d\n", opCode, L, M);
    printf("%d\t%s\t%d\t%d\n", lineNum++, operation, L, M);
}
...
void statement(int level)
{
    if (token.numType == identsym) //identsym
    {
        ...
        if (index == -1 && (symbol_table[index].kind == constant || symbol_table[index].kind == procedure))
            syntaxError(24);
            //syntaxError(9); 
        ...
        expression(level);
        printAssembly("STO", level - symbol_table[index].level, symbol_table[index].addr);
    }
    //else if callsym, appropriated from Appendix D HW4
    else if (token.numType == callsym)
    {
        ...
        if (index == -1 && symbol_table[index].kind != procedure)
            syntaxError(26);
    ...
    else if (token.numType == ifsym) // ifsym
    {
        ...
        // Check for 'fi' after 'then'
        if (token.numType != fisym) // fisym
        {
            syntaxError(505); 
        }
        else
        {
            getToken(); // Consume the 'fi' token and move to the next token
        }
    }
...
void condition(int level)
{
    if (token.numType == oddsym) //oddsym
    {
        getToken();
        expression(level);
        printAssembly("EQL", level, 0);
    }
    else //if (operator == opr_neq)
    {
        expression(level);
        int operator= rel_op();
...
void expression(int level)
{
    ...
    {
        if (token.numType == plussym){
            getToken();
            term(level);
            printAssembly("ADD", level, 0);
        } else if (token.numType == minussym) {
            getToken();
            term(level);
            printAssembly("SUB", level, 0);
        }
    }
}
...
void term(int level)
{
    ...
    while (token.numType == multsym || token.numType == slashsym) // multsym / slashsym
    {
        if (token.numType == multsym) {
            getToken();
            factor(level);
            printAssembly("MUL", level, 0);
        } else if (token.numType == slashsym){
            getToken();
            factor(level);
            printAssembly("DIV", level, 0);
        }
    }
}
...
void factor(int level)
{
    if (token.numType == identsym)
    {
        ...
        if (index == -1 && symbol_table[index].kind == procedure)
            syntaxError(27); 
        if (index != -1)
            printAssembly("LOD", level - symbol_table[index].level, symbol_table[index].addr);
        getToken();
    }
    else if (token.numType == numbersym)
    {
        printAssembly("LIT", 0, atoi(token.value));
        getToken();
    }
